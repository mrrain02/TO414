...
We used the `nba_api` to gather the data from the 2022-2023 Season. Through this we were able to gather all shot information from the Regular Season, as well as personal information regarding each player to make our predictions more accurate. You can find the code used to scrape the information here `https://github.com/mrrain02/TO414.git`
...
saveRDS(Nba_StepBase, "Nba_StepBase_model.rds")
...
```{r}
NbaModel_2 = glm(SHOT_MADE_FLAG ~ log1p(SHOT_DISTANCE) + LOC_X + LOC_Y, data = nba_log_train, family = "binomial")
saveRDS(NbaModel_2, "NbaModel_2.rds")
```

```{r}
# Load the model
NbaModel_2 <- readRDS("NbaModel_2.rds")

# Prepare test data with the original SHOT_DISTANCE variable
test_data <- data.frame(
  SHOT_DISTANCE = 20,  # Original shot distance
  LOC_X = 0,          # Example X coordinate
  LOC_Y = 0           # Example Y coordinate
)

# Apply log1p transformation within the predict function
test_prediction <- predict(NbaModel_2, newdata = test_data, type = "response", transform = list(SHOT_DISTANCE = log1p))
