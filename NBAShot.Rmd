...
# When Train and Split is Seperated, some colleges will not appear so have to remove
nba$COLLEGE <- NULL
nba$COUNTRY <- NULL

# Take Top 3 Countries, Factorize Everything Else

# Take Top 3 Colleges, Factorize Everything Else
...
ann_model <- neuralnet(shot_made_flag1 ~., data=nba_ann_train, hidden = 3)
...
any(is.na(nba_train))
any(is.na(nba_test))
colSums(is.na(nba_train))
colSums(is.na(nba_test))

any(is.na(nba_train_labels))
str(nba_test)

str(nba_train[,141:145])
...
nba_model <- C5.0(as.factor(SHOT_MADE_FLAG) ~ ., data = nba_log_train)
